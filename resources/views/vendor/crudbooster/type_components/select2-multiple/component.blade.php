            ...
            @push('bottom')
                <script type="text/javascript">
                    $(function () {
                        $('#{{$name}}').select2();
                        @if($readonly)
                            $('#{{$name}} option:not(:selected)').attr('disabled', 'disabled');
                        @endif
                    })
                </script>
            @endpush
...
<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    ...
    <div class="{{$col_width?:'col-sm-9'}}">
        <select style="width:100%" class="form-control" id="{{$name}}"
                {{$required}} {{$readonly}} {!!$placeholder!!} {{$disabled}} name="{{$name}}[]" {{ ($form['multiple'])?'multiple="multiple"':'' }} >
            @if($form['dataenum'])
            ...
            @if($form['datatable'])
                ...
                @else
                    @if($form['datatable_ajax'] == false)
                        ...
                        foreach ($result as $r) {
                            $option_label = $r->{$select_title};
                            $option_value = $r->{$select_title};//$select_table_pk
                            if(is_array($value)){
                                $value = implode(" / ",$value);
                            }
                            $selected = (in_array($option_value, array_values(explode(" / ", $value))))?"selected":""; 
