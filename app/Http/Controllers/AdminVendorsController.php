<?php namespace App\Http\Controllers;
	...
	class AdminVendorsController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Incoterms","name"=>"incoterms_id","join"=>"incoterms,incoterms_description"];
			...
			$this->form[] = [
				...
				'validation'=>'required|integer|min:0',
				'width'=>'col-sm-6',
				'datatable'=>'incoterms,incoterms_code',
				'datatable_where'=>"status='ACTIVE'"
			];
	    ...
	    public function actionButtonSelected($id_selected,$button_name) {
	        ...
	        switch ($button_name) {
				case 'set_status_ACTIVE':
					...
					Vendor::whereIn('id',$id_selected)->update([
						'status'=>'ACTIVE', 
	    ...
	    */
		...
		public function getVendorTypeCode($vendor_id) {
			if(is_numeric($vendor_id))
				return Vendor::where('id', $vendor_id)->value('vendor_types_id');
		}

		public function getVendorIncoterms($vendor_id){
			if(is_numeric($vendor_id))
				return Vendor::where('id', $vendor_id)->value('incoterms_id');
		}

		public function getVendorByBrand($brand_id) {
			if(is_numeric($brand_id)){
				return Vendor::where('vendors.brands_id', $brand_id)
					->where("vendors.status","ACTIVE")->orderBy("vendors.vendor_name","ASC")
					->select(DB::raw("CONCAT(vendors.vendor_name,' | ',vendor_types.vendor_type_code) AS vendor_name"), 'vendors.id AS vendor_id')
					->join('vendor_types','vendors.vendor_types_id','=','vendor_types.id')
					->pluck("vendor_name", "vendor_id")->toArray();
			}
