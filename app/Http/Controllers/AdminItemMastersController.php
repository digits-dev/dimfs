...
// 			}
	...
	*/
	...
	public function importSKULegend(Request $request)
	{
	    ini_set('memory_limit', '-1');
		$errors = array();
		$cnt_success = 0;
				...
				foreach ($dataExcel as $key => $value) {
					...
					foreach($segmentations as $segmentation){
        		        ...
        		        if(empty($sku_segment)){
    						array_push($errors, 'Line '.$line_item.': with segmentation "'.$value->$segment.'" at column '.$segmentation->segmentation_description.' not found in submaster.');
    					}
					...
					if(empty($skulegend)){
						array_push($errors, 'Line '.$line_item.': with sku legend "'.$value->sku_legend.'" not found in submaster.');
					}
