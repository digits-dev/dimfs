<?php namespace App\Http\Controllers;
	...
	class AdminItemCurrentPriceChangesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->table = "item_current_price_changes";
			# END CONFIGURATION DO NOT REMOVE THIS LINE

			# START COLUMNS DO NOT REMOVE THIS LINE
			$this->col = [];
			$this->col[] = ["label"=>"Digits Code","name"=>"item_masters_id","join"=>"item_masters,digits_code"];
			$this->col[] = ["label"=>"Is Updated","name"=>"is_updated"];
			$this->col[] = ["label"=>"Current SRP","name"=>"current_srp","callback_php"=>'number_format($row->current_srp,2)'];
			$this->col[] = ["label"=>"Price Change","name"=>"price_change","callback_php"=>'number_format($row->price_change,2)'];
			...
			$this->form[] = ['label'=>'Is Updated','name'=>'is_updated','type'=>'radio','validation'=>'required|integer','width'=>'col-sm-6','dataenum'=>'Array'];
			$this->form[] = ['label'=>'Current SRP','name'=>'current_srp','type'=>'number','validation'=>'required','width'=>'col-sm-6'];
			$this->form[] = ['label'=>'Price Change','name'=>'price_change','type'=>'number','validation'=>'required','width'=>'col-sm-6'];
	    ...
	    */
        ...
        public function updateCurrentSRP(){
            $date_today = Carbon::now();
            $date_now = $date_today->toDateString();
            
            $items = ItemMaster::where('effective_date', $date_now)->get();
            
            foreach($items as $item){
                if(!is_null($item->price_change)){
                    ItemMaster::where('digits_code', $item->digits_code)->update([
                        'current_srp' => $item->price_change,
                        'price_change' => NULL,
                        'effective_date' => NULL
                    ]);
                    
                    ItemCurrentPriceChange::where('item_masters_id', $item->id)
                    ->where('effective_date', $date_now)
                    ->update([
                        'is_updated' => 1,
        ...
        public function updateCosting(){
            ...
            $items = DB::table('item_price_change_approvals')
                ->where('effective_date', $date_now)
                ->where('is_updated', 0)
            ...
            foreach($items as $item){
                // if(!is_null($item->price_change)){
                   ...
                   DB::table('item_price_change_approvals')->where('id', $item->id)
                        ->where('effective_date', $date_now)
                        ->update([
                            'is_updated' => 1,
                            'updated_at' => date('Y-m-d H:i:s')
